...
// NewDiffTracker returns new DiffTracker
...
// DiffTrackingSession struct
type DiffTrackingSession struct {
	ChangedFields []string
	IsNew         bool
}

// NewSession returns new DiffTrackingSession
...
func getChangedFields(struct1 interface{}, struct2 interface{}, useBson bool) ([]string, error) {
	...
	if type1.Kind() != reflect.Struct || type2.Kind() != reflect.Struct {
		return diffs, fmt.Errorf("Can only compare two structs or two pointers to structs: %s vs %s", type1.Kind(), type2.Kind())
	}
